var db = window.openDatabase("LeeWee", "1.0", "LeeWee Shop", 200000);
...
function fetch_database() {
    db.transaction(function(tx) {
        ...
        tx.executeSql(query, ['ripndip t-shirt', 'ripndip t-shirt'], fetch_transactions_success("ripndip t-shirt"), transaction_error);
        tx.executeSql(query, ['stussy t-shirt', 'stussy t-shirt'], fetch_transactions_success("stussy t-shirt"), transaction_error);
        tx.executeSql(query, ['thrasher t-shirt', 'thrasher t-shirt'], fetch_transactions_success("thrasher t-shirt"), transaction_error);


        query = `INSERT INTO account(username, password, status) VALUES(?, ?, 1)`;

        tx.executeSql(query, ['leewee@gmail.com', '123456'], fetch_transactions_success("'leewee@gmail.com"), transaction_error);
    });
}